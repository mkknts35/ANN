    ...
    )
{
    ...
    for (unsigned int i = 0; i < output.size(); i++) {
        m_layer.push_back(make_shared<sigmoid>(input, output.at(i), false));
    }
}
//============================================================================
layer::layer(int size, vector <shared_ptr<double>> input)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        m_layer.push_back(make_shared<sigmoid>(input, out));
...
{
    for (unsigned int i = 0; i < m_layer.size(); i++) {
        weights->push_back(m_layer.at(i)->getWeights());
    }
    if (m_next != nullptr) {
...
{
    for (unsigned int i = 0; i < m_layer.size(); i++) {
        m_layer.at(i)->setWeights(*weights);
        weights++;
    }
    if (m_next != nullptr) {
